...
public class Operation {
                ...
                case 1:
                    if (!CompteCourant.liste_compteCourant.isEmpty()) {
                        ...
                        if (ValideIndiceCourant(indiceCourant)) {
                            menuChoixOperationCourant(indiceCourant);
                        } else {
    ...
    public void menuChoixOperationEpargne(int indice) {
        ...
        do {
            ...
            switch (choix) {
                case 1:
                    System.out.println("Veuillez entrer le montant que vous souhaitez déposer > ");
                    double montantDepot = scanner.nextDouble();
                    operationDepotEpargne(montantDepot, indice);
                    break;

                case 2:
                    System.out.println("Veuillez entrer le montant que vous souhaitez retirer > ");
                    double montantRetrait = scanner.nextDouble();
                    operationRetirerEpargne(montantRetrait, indice);
                    break;
                case 3:
                    System.out.println("Veuillez entrer le montant que vous souhaitez verser > ");
    ...
    public void operationVerser(double montantVerser, int indice) {
        ...
        if (CompteEpargne.liste_compteEpargne.get(indice).CalculSoldeActuel(CompteEpargne.liste_compteEpargne.get(indice)) >= montantVerser) {
                ...
                case 1:
                    if (!CompteEpargne.liste_compteEpargne.isEmpty()) {
                        ...
                        if (ValideIndiceEpargne(indiceEpargne)) {
                            ...
                            double nouveauSoldeEpargne1 = CompteEpargne.liste_compteEpargne.get(indice).CalculSoldeActuel(CompteEpargne.liste_compteEpargne.get(indice));

                            CompteEpargne.liste_compteEpargne.get(indice).setSolde(nouveauSoldeEpargne1 - montantVerser);

                            double nouveauSoldeEpargne = CompteEpargne.liste_compteEpargne.get(indiceEpargne).CalculSoldeActuel(CompteEpargne.liste_compteEpargne.get(indiceEpargne));


                            CompteEpargne.liste_compteEpargne.get(indiceEpargne).setSolde(nouveauSoldeEpargne + montantVerser);
            ...
            System.out.println("cette operation est impossible");
        }
    }

    //Menu de choix pour les operations sur les comptes courants
    public void menuChoixOperationCourant(int indice) {
        Scanner scanner = new Scanner(System.in);
        int choix = 0;
        do {
            System.out.println("_________________Menu de Choix de Type d'operation_______________________");
            ...
            switch (choix) {
                case 1:
                    System.out.println("Veuillez entrer le montant que vous souhaitez déposer > ");
                    double montantDepot = scanner.nextDouble();
                    operationDepotCourant(montantDepot, indice);
                    break;

                case 2:
                    System.out.println("Veuillez entrer le montant que vous souhaitez retirer > ");
                    double montantRetrait = scanner.nextDouble();
                    operationRetirerCourant(montantRetrait, indice);
                    break;
                case 3:
                    System.out.println("Veuillez entrer le montant que vous souhaitez verser > ");
                    double montantVerser = scanner.nextDouble();
                    operationVerserCourant(montantVerser, indice);
                case 4:
                    break;
            }
        } while (choix != 4);
        ...
        public void operationVerserCourant ( double montantVerser, int indice){
            Scanner scanner = new Scanner(System.in);
            if (CompteCourant.liste_compteCourant.get(indice).CalculSoldeActuel(CompteCourant.liste_compteCourant.get(indice)) >= montantVerser) {
                    ...
                    case 1:
                        if (!CompteEpargne.liste_compteEpargne.isEmpty()) {
                            ...
                            if (ValideIndiceEpargne(indiceEpargne)) {
                                ...
                                String CompteAssocie3 = "N : " + CompteCourant.liste_compteCourant.get(indice).getNumero() + " Compte Courant";

                                String type3 = "virement";

                                ListeOpertion.add(new Operation(type3, montant, date3, CompteAssocie3));

                                double nouveauSoldeCourant = CompteCourant.liste_compteCourant.get(indice).CalculSoldeActuel(CompteCourant.liste_compteCourant.get(indice));

                                CompteCourant.liste_compteCourant.get(indice).setSolde(nouveauSoldeCourant - montantVerser);
